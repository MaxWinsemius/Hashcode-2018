...
class Simulation:
    ...
    def __init__(self, file):
        with open(file) as f:
            ...
            self.rides, self.long_rides = self.split_rides(self.all_rides)
            self.total_rides = len(self.rides) + len(self.long_rides)

    def split_rides(self, rides):
        total_len = len(rides)
        short_rides = rides[:(total_len//10)*9]
        long_rides = rides[(total_len//10)*9:]
